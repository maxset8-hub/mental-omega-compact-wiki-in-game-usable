#!/usr/bin/env python3
"""
Mental Omega Arsenal - Comprehensive Unit Database Application
A single-file application for browsing Mental Omega mod units, structures, and support powers.
Features always-on-top overlay, hierarchical navigation, search, and unit comparison.
"""
...
class ThemeManager:
    ...
    def get_font(self, size_delta: int = 0, weight: str = None) -> font.Font:
        ...
        size = self.custom_settings["font_size"] + size_delta
        if weight is None:
            weight = self.custom_settings["font_weight"]
...
class IconManager:
    ...
    def get_icon(self, icon_path: str, size: Tuple[int, int] = (32, 32)) -> Optional[ImageTk.PhotoImage]:
        ...
        try:
            ...
            # Apply scaling
            scale = self.theme_manager.custom_settings["icon_scale"]
...
class MentalOmegaArsenal:
    ...
    def create_navigation_toolbar(self):
        ...
        toolbar = tk.Frame(self.content_frame, bg=DEFAULT_THEME["title_bg"], height=40)
        toolbar.pack(fill=tk.X, pady=(0, 5))
        toolbar.pack_propagate(False)
        
        # Navigation buttons
        nav_frame = tk.Frame(toolbar, bg=DEFAULT_THEME["title_bg"])
        ...
        self.breadcrumb_label.pack(side=tk.LEFT, padx=20, pady=5)
    ...
    def setup_shortcuts(self):
        ...
        self.root.bind("<Control-h>", lambda e: self.show_factions())
    ...
    def update_breadcrumb(self):
        ...
        breadcrumb = " > ".join(self.current_path) if self.current_path else "Home"
    ...
    def show_factions(self):
        ...
        # Create faction grid
        factions_frame = tk.Frame(self.main_content, bg=DEFAULT_THEME["bg"])
        factions_frame.pack(fill=tk.BOTH, expand=True, padx=20, pady=20)
        ...
        for faction_name in FACTION_HIERARCHY:
            faction_frame = tk.Frame(factions_frame, bg=DEFAULT_THEME["bg"])
            faction_frame.pack(fill=tk.X, pady=5)
            
            # Get faction icon
            icon = self.icon_manager.get_faction_icon(faction_name, (48, 48))
            ...
            btn = tk.Button(
                ...
                command=lambda f=faction_name: self.show_subfactions(f),
                bd=0,
                highlightthickness=0,
                anchor=tk.W,
                pady=10
    ...
    def show_subfactions(self, faction: str):
        """Show subfactions for a faction"""
        self.current_path = [faction]
        self.update_breadcrumb()
        self.clear_main_content()
        
        subfactions_frame = tk.Frame(self.main_content, bg=DEFAULT_THEME["bg"])
        subfactions_frame.pack(fill=tk.BOTH, expand=True, padx=20, pady=20)
        
        title = tk.Label(
            subfactions_frame,
            text=f"{faction} - Select Subfaction",
            bg=DEFAULT_THEME["bg"],
            fg=DEFAULT_THEME["fg"],
            font=self.theme_manager.get_font(4, "bold")
        )
        ...
        for subfaction_name in FACTION_HIERARCHY[faction]["subfactions"]:
            subfaction_frame = tk.Frame(subfactions_frame, bg=DEFAULT_THEME["bg"])
            subfaction_frame.pack(fill=tk.X, pady=5)
            
            # Get subfaction icon
            icon_name = FACTION_HIERARCHY[faction]["subfactions"][subfaction_name]["icon"]
            icon = self.icon_manager.get_icon(f"{icon_name}.webp", (32, 32))
            
            btn = tk.Button(
                subfaction_frame,
                text=f"  {subfaction_name}",
                image=icon,
                compound=tk.LEFT if icon else None,
                bg=DEFAULT_THEME["button_bg"],
                fg=DEFAULT_THEME["button_fg"],
                font=self.theme_manager.get_font(2),
                command=lambda f=faction, s=subfaction_name: self.show_categories(f, s),
                bd=0,
                highlightthickness=0,
                anchor=tk.W,
                pady=10
    ...
    def show_categories(self, faction: str, subfaction: str):
        """Show categories for a subfaction"""
        self.current_path = [faction, subfaction]
        self.update_breadcrumb()
        self.clear_main_content()
        
        categories_frame = tk.Frame(self.main_content, bg=DEFAULT_THEME["bg"])
        categories_frame.pack(fill=tk.BOTH, expand=True, padx=20, pady=20)
        
        title = tk.Label(
            categories_frame,
            text=f"{subfaction} - Select Category",
            bg=DEFAULT_THEME["bg"],
            fg=DEFAULT_THEME["fg"],
            font=self.theme_manager.get_font(4, "bold")
        )
        ...
        for category in UNIT_CATEGORIES:
            ...
            category_frame = tk.Frame(categories_frame, bg=DEFAULT_THEME["bg"])
            category_frame.pack(fill=tk.X, pady=5)
            
            btn = tk.Button(
                category_frame,
                text=f"  {category} ({len(units)})",
                bg=DEFAULT_THEME["button_bg"],
                fg=DEFAULT_THEME["button_fg"],
                font=self.theme_manager.get_font(2),
                command=lambda f=faction, s=subfaction, c=category: self.show_units(f, s, c),
                bd=0,
                highlightthickness=0,
                anchor=tk.W,
                pady=10
    ...
    def show_units(self, faction: str, subfaction: str, category: str):
        """Show units for a category"""
        self.current_path = [faction, subfaction, category]
        self.update_breadcrumb()
        self.clear_main_content()
        
        units_frame = tk.Frame(self.main_content, bg=DEFAULT_THEME["bg"])
        units_frame.pack(fill=tk.BOTH, expand=True, padx=20, pady=20)
        
        title = tk.Label(
            units_frame,
            text=f"{category} - {subfaction}",
            bg=DEFAULT_THEME["bg"],
            fg=DEFAULT_THEME["fg"],
            font=self.theme_manager.get_font(4, "bold")
        )
        ...
        # Units list with scrollbar
        list_frame = tk.Frame(units_frame, bg=DEFAULT_THEME["bg"])
        list_frame.pack(fill=tk.BOTH, expand=True)
        
        # Create scrollable frame
        canvas = tk.Canvas(list_frame, bg=DEFAULT_THEME["bg"], highlightthickness=0)
        ...
        # Enable mouse wheel scrolling anywhere on the canvas
        ...
        for unit in sorted(units, key=lambda u: u.name):
            unit_frame = tk.Frame(scrollable_frame, bg=DEFAULT_THEME["button_bg"])
            unit_frame.pack(fill=tk.X, pady=2, padx=5)
            
            # Unit icon - try multiple path variants
            icon_variants = [
                f"{faction.replace(' ', '_')}/subfaction/{subfaction}/{category}/icons/{unit.icon_filename}",
                f"{faction}/subfaction/{subfaction}/{category}/icons/{unit.icon_filename}",
                f"{faction.replace(' ', '_')}/subfaction/{subfaction.replace(' ', '_')}/{category.replace(' ', '_')}/icons/{unit.icon_filename}",
                f"{faction}/{category}/icons/{unit.icon_filename}",
                f"{faction.replace(' ', '_')}/{category.replace(' ', '_')}/icons/{unit.icon_filename}"
            ]
            
            icon = None
            for icon_path in icon_variants:
                icon = self.icon_manager.get_icon(icon_path, (32, 32))
                if icon:
                    break
            
            # Unit button
            btn_text = f"  {unit.name}"
            btn = tk.Button(
                unit_frame,
                text=btn_text,
                image=icon,
                ...
                command=lambda u=unit: self.show_unit_details(u),
                bd=0,
                highlightthickness=0,
                anchor=tk.W,
                pady=8
            ...
            compare_btn = tk.Button(
                ...
                fg=DEFAULT_THEME["fg"],
                width=3,
                command=lambda u=unit: self.toggle_comparison(u),
                bd=0,
                highlightthickness=0
            )
    ...
    def show_unit_details(self, unit: UnitData):
        ...
        self.current_path.append(unit.name)
        self.update_breadcrumb()
        self.clear_main_content()
    ...
    def create_article_tables(self, parent, article_tables: List[Dict]):
        """Create article tables for units with special data"""
        if not article_tables:
            return
            
        # Create a single container for all tables
        ...
        canvas = tk.Canvas(combined_frame, bg=DEFAULT_THEME["bg"], highlightthickness=0)
        ...
        # Display all tables in the same container
        for table_index, table in enumerate(article_tables):
            # Table title
            table_title = table.get("title", f"Table {table_index + 1}")
            title_label = tk.Label(
            ...
            # Table container
            table_container = tk.Frame(scrollable_frame, bg=DEFAULT_THEME["bg"])
            table_container.pack(fill=tk.X, padx=10, pady=(0, 10))
            
            # Headers
            header_frame = tk.Frame(table_container, bg=DEFAULT_THEME["select_bg"])
            ...
            # Rows
            for row in rows:
                row_frame = tk.Frame(table_container, bg=DEFAULT_THEME["bg"])
                row_frame.pack(fill=tk.X, pady=1)
                
                for i, cell in enumerate(row):
                    cell_text = str(cell).replace("\n", " ")
                    # Handle long text by splitting into bullet points, but show ALL content
                    if "," in cell_text and len(cell_text) > 30:
                        # Split comma-separated items into bullet points
                        items = [item.strip() for item in cell_text.split(",")]
                    ...
                    cell_label.pack(side=tk.LEFT, padx=2, fill=tk.BOTH, expand=True)
    ...
    def display_search_results(self, results: List[SearchItem]):
        ...
        for result in results:
            ...
            compare_btn = tk.Button(
                ...
                fg=DEFAULT_THEME["fg"],
                width=3,
                command=lambda r=result: self.toggle_comparison(r.unit_data),
                bd=0,
                highlightthickness=0
            )
            compare_btn.pack(side=tk.RIGHT, padx=5)
    ...
    def toggle_comparison(self, unit: UnitData):
        ...
        if unit in self.comparison_units:
            ...
            print(f"Removed {unit.name} from comparison")  # Simple feedback
        else:
            self.comparison_units.append(unit)
            print(f"Added {unit.name} to comparison")  # Simple feedback
    
    def show_comparison(self):
        """Show unit comparison view"""
        if len(self.comparison_units) == 0:
            self.current_path = ["Comparison"]
            self.update_breadcrumb()
            self.clear_main_content()
    ...
    def create_comparison_properties(self, parent, unit: UnitData, comparison_canvases=None):
        ...
        # Enable normal mouse wheel scrolling for vertical scroll (not synchronized)
        def _on_mousewheel(event):
            canvas.yview_scroll(int(-1*(event.delta/120)), "units")
    ...
    def clear_comparison(self):
        ...
        # Reset window to default size when clearing comparison
        self.root.geometry(f"{DEFAULT_WINDOW_SIZE[0]}x{DEFAULT_WINDOW_SIZE[1]}")
    ...
    def remove_from_comparison(self, unit: UnitData):
        ...
        if unit in self.comparison_units:
            self.comparison_units.remove(unit)
            # Adjust window size based on remaining units
            if len(self.comparison_units) == 0:
                self.root.geometry(f"{DEFAULT_WINDOW_SIZE[0]}x{DEFAULT_WINDOW_SIZE[1]}")
            else:
                window_width = min(2000, max(1000, 400 + len(self.comparison_units) * 300))
    ...
    def show_settings(self):
        ...
        settings_frame = tk.Frame(self.main_content, bg=DEFAULT_THEME["bg"])
        ...
        # Font size setting
        font_frame = tk.Frame(appearance_frame, bg=DEFAULT_THEME["bg"])
        ...
        # Icon scale setting
        icon_frame = tk.Frame(appearance_frame, bg=DEFAULT_THEME["bg"])
    ...
    def apply_window_size(self):
        ...
        # Update default window size for future reference
    ...
    def go_back(self):
        ...
        # Reset window size when leaving comparison view
        if self.current_path and self.current_path[0] == "Comparison":
            if hasattr(self, '_original_window_size'):
                self.root.geometry(f"{self._original_window_size[0]}x{self._original_window_size[1]}")
            else:
                self.root.geometry(f"{DEFAULT_WINDOW_SIZE[0]}x{DEFAULT_WINDOW_SIZE[1]}")
        ...
        # Navigate based on current path length
        if len(self.current_path) == 0:
            self.show_factions()
        elif len(self.current_path) == 1:
            self.show_subfactions(self.current_path[0])
        elif len(self.current_path) == 2:
            self.show_categories(self.current_path[0], self.current_path[1])
        elif len(self.current_path) == 3:
            self.show_units(self.current_path[0], self.current_path[1], self.current_path[2])
        else:
            # Unit details - need to find the unit
            unit_name = self.current_path[-1]
            for unit in self.data_manager.units.values():
                if unit.name == unit_name:
                    self.show_unit_details(unit)
                    break
    
    def close_current_dialog(self):
        """Close current dialog/view"""
        if len(self.current_path) > 1:
            self.go_back()
    ...
    def on_collapse(self, is_collapsed: bool):
        ...
        if is_collapsed:
            ...
            # IMPORTANT: Remove minimum size constraint to allow collapsing
            self.root.minsize(1, 1)  # Set to minimal possible size
            ...
            # IMPORTANT: Restore minimum size constraint after expanding
            self.root.minsize(*MIN_WINDOW_SIZE)
